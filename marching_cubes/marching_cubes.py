...
class Surface():
    ...
    def calculate_surface_area(self, grid, voxel_size=(1, 1, 1), min_coords=(0,0,0), level=None, out=None, maxlen=1, plot=True):
    ...
    def projected_area_and_plot(self, vertices, eps=0.5, min_samples=3, max_edge_length=1, out=None, plot=True):
    ...
    def plot_surface(self, grid, verts, faces, voxel_size=(1, 1, 1), min_coords=(0, 0, 0), out=None):
        ...
        xs ,ys, zs = [min_coords[0], min_coords[0] + grid.shape[0] * voxel_size[0]], [min_coords[1], min_coords[1] + grid.shape[1] * voxel_size[1]], [min_coords[2], min_coords[2] + grid.shape[2] * voxel_size[2]]
        ax.set_xlim(*xs)
        ax.set_ylim(*ys)
        ax.set_zlim(*zs)
        ...
        xrange, yrange, zrange =  xs[1]-xs[0], ys[1]-ys[0], zs[1]-zs[0]
        xf, yf, zf = xrange/xrange, yrange/xrange, zrange/xrange
        ax.set_box_aspect(np.array([xf,yf,zf]))
        ...
        contourf = ax2.contourf(xi, yi, zi, levels=15, cmap='viridis') 
        background_color = plt.cm.viridis(0)
        ax2.set_fc(background_color)
        ax2.set_xlabel('x (nm)')
        ax2.set_ylabel('y (nm)')
        ax2.set_aspect('equal')
        # Add a colorbar to show the mapping from colors to values
        cbar = fig2.colorbar(contourf, orientation='horizontal', pad=0.1, shrink=0.8, aspect=30, location="top")
    ...
    def calculate_marching_cubes(self, voxel_size=(1, 1, 1), sigma=False, layers_to_ignore=False, level=0.5, out=None, maxlen=1):
